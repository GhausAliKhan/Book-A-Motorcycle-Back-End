class Api::V1::ReservationsController < ApplicationController
  ...
  def index
    ...
    if @reservations.size.positive?
      ...
      render json: { success: true, message: 'There is no Reservation!' }, status: :ok
    end
  end
  ...
  def destroy
    if Reservation.exists?(params[:id])
      ...
      render json: { error: true, message: 'Could not remove this reservation' },
  ...
  def reservation_params
    params.require(:reservation).permit(:motorcycle_id, :city, :date, :duration, :total)
  end
end
